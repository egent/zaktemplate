...
<thead> 
<tr>
<th class="filter-select filter-exact">Населенный пункт</th>
<th>Сотоки</th>
<th class="sorter-false filter-false">Этаж</th>
<th class="sorter-false filter-false">Пл.</th>
<th class="sorter-false filter-false">От. Эл. Во.</th>
...
<td>{{ item.floor }}</td>
<td>{{ item.square }}</td>
<td>{{ item.otop != '+' ? '-' : '+' }} / {{ item.elect != '+' ? '-' : '+'}} / {{ item.vo != '+' ? '-' : '+' }}</td>
<td>
...
{% endblock %}
{% block footer_text %}Дома, коттеджи Набережные Челны{% endblock %}